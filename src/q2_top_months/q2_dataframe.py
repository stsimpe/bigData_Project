...
def run_query(crimes: DataFrame) -> DataFrame:
    ...
    with_date = (
        ...
        # Το DATE OCC είναι της μορφής "MM/dd/yyyy 12:00:00 AM" — η ώρα είναι
        # πάντα μεσάνυχτα (dummy). Κρατάμε μόνο το date κομμάτι πριν το κενό
        # και το παρσάρουμε ως ημερομηνία. Έτσι αποφεύγουμε τον αυστηρό
        # AM/PM parser του Spark 3 (που με 'hh:mm:ss a' γύριζε null σε όλες
        # τις γραμμές και άδειαζε το αποτέλεσμα).
        .withColumn(
            "_d", F.to_date(F.substring_index(F.col("DATE OCC"), " ", 1), "MM/dd/yyyy")
        )
        .filter(F.col("_d").isNotNull())  # πέταξε rows που δεν παρσαρανε
